...
public class ProvaveisNumeros_Test {
	...
	public void testaBuscaFrequenciaSemCarregar() {
		...
		try {
			...
			Assert.assertEquals("É necessário carregar os dados antes de buscar sua frequência.", e.getMessage());
		}
	}
	...
	public void testaValidacao_10() throws IOException {
		...
		try {
			...
			Assert.assertTrue(e instanceof IllegalStateException);
			Assert.assertEquals("O número inicial do concurso não pode ser maior que o número final.", e.getMessage());

			Assert.assertFalse(configuracao.isValido());
			Assert.assertEquals("O número inicial do concurso não pode ser maior que o número final.", configuracao.getMotivoInvalido());
		}
	}
}
